...
export const NowPlayingBar: React.FC<NowPlayingBarProps> = ({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-[100px] bg-gradient-to-t from-black via-black/95 to-black/80 backdrop-blur-3xl border-t border-white/10 grid grid-cols-[1fr_2fr_1fr] items-center px-6 gap-6 z-[1000] max-lg:grid-cols-[1fr_1.5fr_0.5fr] max-md:h-auto max-md:grid-cols-1 max-md:py-4 max-md:gap-4 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
      {/* Track Info */}
      <div className="flex items-center gap-4 min-w-0">
        <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 bg-gradient-to-br from-primary/20 to-purple-600/20 shadow-lg group-hover:shadow-primary/30 transition-all duration-300">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-base font-bold text-white overflow-hidden text-ellipsis whitespace-nowrap mb-1 hover:text-primary transition-colors duration-300 cursor-pointer">
            {track.name}
          </div>
          <div className="text-sm text-white/50 overflow-hidden text-ellipsis whitespace-nowrap font-medium">
            {track.artist}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 items-center">
        <div className="flex items-center gap-3">
          ...
          <button 
            className="w-12 h-12 bg-gradient-to-br from-primary via-purple-500 to-purple-600 text-white border-none cursor-pointer rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 hover:shadow-[0_0_30px_rgba(147,51,234,0.6)] active:scale-95 shadow-xl" 
            onClick={onPlayPause} 
            aria-label={playerState.isPlaying ? 'Pause' : 'Play'}
          >
            {playerState.isPlaying ? <Pause size={26} fill="currentColor" /> : <Play size={26} fill="currentColor" className="ml-1" />}
          </button>
          <button 
            className="bg-transparent border-none text-white/70 cursor-pointer p-2.5 flex items-center justify-center rounded-xl transition-all duration-300 hover:text-white hover:bg-white/10 hover:scale-110 active:scale-95" 
            onClick={onNext} 
        ...
        <div className="flex items-center gap-3 w-full max-w-[650px]">
          <span className="text-xs text-white/60 font-mono min-w-[42px] text-center font-semibold">
            {formatTime(playerState.currentTime)}
          </span>
          <div className="relative flex-1 h-2 bg-white/10 rounded-full overflow-hidden group backdrop-blur-sm">
            <div
